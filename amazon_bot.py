...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Hi! Send me an Amazon facility code (e.g., FWA4) and I'll tell you if it accepts early arrivals.\n"
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status == "Unknown":
        await update.message.reply_text(
            f"{prefix}{code}: Not found in my list. It might accept early—check the latest facility info."
        )
    else:
        emoji = "✅" if "Early" in status else "❌" if "Only" in status else "⚖️"
        await update.message.reply_text(f"{prefix}{code}: {status} {emoji}")

def main():
    token = os.environ.get("BOT_TOKEN")
    if not token:
        # This will show clearly in Railway logs
        raise RuntimeError("BOT_TOKEN is missing. Go to Railway → Variables → add BOT_TOKEN.")
    ...
    logger.info("Bot running (polling). If it crashes, open Railway 'View logs' to see the error.")
    # This also tries to clear any old webhook conflicts:
